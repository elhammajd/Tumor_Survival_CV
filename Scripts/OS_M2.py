...
def OS(dataset1, dataset2, dataset3):
    ...
    f1 = open('./Results/M2_OS_Bestcut'+dataset1+'.jpg', 'w')
    f1.truncate()
    f1.close()

    f2 = open('./Results/M2_WeibullAFT_'+dataset1+'.jpg', 'w')
    f2.truncate()
    f2.close()
    ...
    coef= pd.read_csv('./Results/M2_Coefs_'+dataset2+'.csv')
    coef.columns= ['id','Coefficients','feature','Model']
    coef1= pd.read_csv('./Results/M2_Coefs_'+dataset3+'.csv')
    coef1.columns= ['id','Coefficients','feature','Model']


    data['PC2']= ((coef.loc[0,'Coefficients']+coef1.loc[0,'Coefficients'])/2)*data[['PC2']]
    data['SEX']= ((coef.loc[1,'Coefficients']+coef1.loc[1,'Coefficients'])/2)*data[['SEX']]
    data['AGE']= ((coef.loc[2,'Coefficients']+coef1.loc[2,'Coefficients'])/2)*data[['AGE']]
    data['SMOKE']= ((coef.loc[3,'Coefficients']+coef1.loc[3,'Coefficients'])/2)*data[['SMOKE']]
    data['WEIGHT']= ((coef.loc[4,'Coefficients']+coef1.loc[4,'Coefficients'])/2)*data[['WEIGHT']]
    data['RACE']= ((coef.loc[5,'Coefficients']+coef1.loc[5,'Coefficients'])/2)*data[['RACE']]
    data['intercept']= ((coef.loc[6,'Coefficients']+coef1.loc[6,'Coefficients'])/2)
    data_score= data[['PC2', 'SEX', 'AGE', 'SMOKE', 'WEIGHT', 'RACE']]
    ...
    plt.savefig('./Results/'+dataset1+'_Test_OS_Standard_Train_'+dataset2+'.jpg', bbox_inches='tight')
    ...
    wb_aft.plot()
    plt.savefig('./Results/'+dataset1+'_M2_Test_WeibullAFT_Train_'+dataset2+'.jpg', bbox_inches='tight')
    #===========================================[Cox Regression]=============================================
    print('Cox Regression', dataset1)
    cph = CoxPHFitter(alpha=0.05)
    cph.fit(data2, duration_col = 'time', event_col = 'event')
