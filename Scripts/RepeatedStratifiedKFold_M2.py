...
#split data
    ...
    for k in n_splits:
        repeats = range(2,6)
        for r in repeats:
            cv_outer = RepeatedStratifiedKFold(n_splits=k, n_repeats=r, random_state=1)
            for train_ix, test_ix in cv_outer.split(X, y): 
                X_train, X_test = X[train_ix, :], X[test_ix, :] 
                y_train, y_test = y[train_ix], y[test_ix]
                cv_inner = RepeatedStratifiedKFold(n_splits=5, n_repeats=3, random_state=1)
            ...
            feature_names=['PC2', 'SEX', 'AGE', 'SMOKE', 'WEIGHT', 'DOSE', 'intercept'] 
