...
import pylab as pl
import numpy as np 
from Outputs_M2 import *
#================================================[HoldOut]===========================================
SEED=42
def make_dataset(dataset):  

    f1 = open('./Results/M2_Coefs_'+dataset+'.csv', 'a') 
    f1.truncate() 
    f1.close() 

    f2 = open('./Results/M2_Performance_'+dataset+'.csv', 'a') 
    f2.truncate() 
    f2.close()

    f3 = open('./Results/M2_Probability_'+dataset+'.csv', 'a')
    f3.truncate()
    f3.close()

    f4 = open('./Results/M2_Pearson_residuals_'+dataset+'.csv', 'a')
    f4.truncate()
    f4.close()
...
def method(dataset, model_name1, model_name2):
#split data
